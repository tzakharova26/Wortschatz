...
from bot import learn as learn_module
...
from bot.learn import (
    ARTICLE as LEARN_ARTICLE_TYPE,
)
from bot.learn import (
    MC as LEARN_MC_TYPE,
)
from bot.learn import (
    PLURAL as LEARN_PLURAL_TYPE,
)
from bot.learn import (
    SHOW as LEARN_SHOW_TYPE,
)
from bot.learn import (
    TYPED as LEARN_TYPED_TYPE,
)
from bot.learn import (
    VERB_FORM as LEARN_VERB_FORM_TYPE,
)
...
def _build_learn_step_markup(step) -> InlineKeyboardMarkup | None:
    """Inline keyboard for the current learn step. Typed/verb_form steps return None."""
    if step.step_type == LEARN_SHOW_TYPE:
        return InlineKeyboardMarkup(
            [[InlineKeyboardButton("Got it", callback_data=f"{CB_LEARN_SHOW}ok")]]
        )
    if step.step_type == LEARN_MC_TYPE and step.options:
        return InlineKeyboardMarkup(
            [[InlineKeyboardButton(o, callback_data=f"{CB_LEARN_MC}{o}")] for o in step.options]
        )
    if step.step_type == LEARN_ARTICLE_TYPE and step.options:
        return InlineKeyboardMarkup(
...
async def _send_learn_step(reply_target, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    session: learn_module.LearnSession = context.user_data["learn_session"]
...
async def _start_learn_session(
    ...
    session = learn_module.build_session(user_id, words, all_user_words)
...
async def learn_button_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    session: learn_module.LearnSession | None = context.user_data.get("learn_session")
    if not session or session.is_finished:
    ...
    if data.startswith(CB_LEARN_SHOW):
        ...
        correct = learn_module.check_answer(step, user_answer)
...
async def learn_text_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handles TYPED and VERB_FORM steps."""
    session: learn_module.LearnSession | None = context.user_data.get("learn_session")
    if not session or session.is_finished:
    ...
    step = session.current_step
    if step.step_type not in (LEARN_TYPED_TYPE, LEARN_VERB_FORM_TYPE, LEARN_PLURAL_TYPE):
...
async def _finish_learn(reply_target, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    conn = _get_conn(context)
    try:
        graduated = await learn_module.apply_graduations(conn, session)
    except Exception as e:
        log_user_error(
            logger,
            session.user_id,
    ...
    summary = learn_module.format_summary(session)
    if graduated == -1:
...
async def learn_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if session and session.graduated_words():
        try:
            await learn_module.apply_graduations(_get_conn(context), session)
            summary = learn_module.format_summary(session)
            await update.message.reply_text(
